...
安全凭据存储方案实施前需要验证 Windows 和 macOS 的可用系统能力；未完成安全存储时，只允许从环境变量或用户临时输入读取，不做明文持久化。
...
- 首批支持的本地模型运行时和远程提供方。
- Windows 与 macOS 安全凭据存储的最终实现。
- 提交正文默认生成还是按需生成。
- 是否读取仓库规范文件，以及允许读取的文件白名单。
- 文件级规划是否默认分析未暂存改动。
- 计划执行前是否提供可配置的项目测试命令，但模型不得自行决定或运行命令。
- 首轮真实试用仓库和人工接受率记录方式。

这些决策不阻塞 P0 的协议与测试基座建设。
