...
Window {
    ...
    readonly property bool probeHasMore: historyView.hasMore
...
def _run_probe(repo: Path) -> int:
    ...
    try:
        ...
        if root.property("probePendingLog") or root.property("probeCommitCount") != 60:
            raise AssertionError(
                {
                    "phase": "pagination_not_applied_immediately",
                    "count": root.property("probeCommitCount"),
        ...
        if root.property("probePendingLog"):
            raise AssertionError(
                {
                    "phase": "pagination_unexpectedly_deferred",
                    "count": root.property("probeCommitCount"),
                    "loading": root.property("probeLoading"),
                    "samples": pagination_samples,
                }
        ...
        if root.property("probeCommitCount") != 60 or root.property("probeLoading"):
            raise AssertionError(
                {
                    "phase": "pagination_not_finished",
                    "count": root.property("probeCommitCount"),
                    "loading": root.property("probeLoading"),
                    "pending": root.property("probePendingLog"),
                    "samples": pagination_samples,
...
def _run_auto_probe(repo: Path) -> int:
    from PySide6.QtCore import QMetaObject, QObject, Qt
    from PySide6.QtQml import QQmlApplicationEngine
    from PySide6.QtWidgets import QApplication
    from prismqml import register_types
...
if __name__ == "__main__":
    if len(sys.argv) != 3 or sys.argv[1] not in {"--probe", "--auto-probe"}:
        raise SystemExit(
            "usage: test_history_scroll_qml.py --probe|--auto-probe REPO"
        )
    runner = _run_auto_probe if sys.argv[1] == "--auto-probe" else _run_probe
    raise SystemExit(runner(Path(sys.argv[2])))
